    ...
    return (
        <div className="user-profile">
            ...
            <div style={{display: 'flex'}}>
                ...
                <Typography z-index="1" fontWeight={600} color="#FF2625"
                            sx={{opacity: '0.40', display: {lg: 'block', xs: 'none'}, fontSize: '100px'}}>
                    - {isEditing ? <textarea rows="5" cols="60" value={user.name}
                                             onChange={(e) => handleChange('name', e.target.value)}/> : user.name} - <br/> Iron
                    Alliance Member
                </Typography>
            </div>
            <p style={{color: '#666', lineHeight: '1.5'}}>
                ...
                old <br/>
                <strong>Weight:</strong> {isEditing ? <input value={user.weight}
                                                             onChange={(e) => handleChange('weight', e.target.value)}/> : user.weight} lbs <br/>
                <strong>Height:</strong> {isEditing ? <input value={user.height}
                                                             onChange={(e) => handleChange('height', e.target.value)}/> : user.height} in <br/>
                <strong>Email:</strong> {isEditing ?
                <textarea value={user.email} onChange={(e) => handleChange('email', e.target.value)}
                          style={{height: '50px'}}/> : user.email} <br/>
            </p>
            <br/>
            <p style={{color: '#999', fontSize: '30px', margin: '10px 0'}}>Fitness Goals: {isEditing ?
                <textarea rows="5" cols="60" value={user.goals}
                          onChange={(e) => handleChange('goals', e.target.value)}/> : user.goals}</p>
            {isEditing ? (
                <div>
            ...
            <div style={{display: 'flex', justifyContent: 'center', marginTop: '20px'}}>
                <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                    ...
                    <img src={beforePic} alt="Before Picture"
                ...
                <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', marginLeft: '30px'}}>
                    ...
                    <img src={afterPic} alt="Latest Picture"
                         style={{width: '400px', height: '450px', marginBottom: '20px'}}/>
                </div>
            </div>
            <div style={{display: 'flex', justifyContent: 'center', marginTop: '20px'}}>
